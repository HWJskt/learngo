...
// Scrape : 함수이름을 대문자로 시작해서 import할수있게함
func Scrape(term string) {
	var baseURL string = "https://search.shopping.naver.com/book/search?bookTabType=ALL&pageIndex=1&pageSize=40&query=" + term + "&sort=REL"
...
func getPage(page int, term string, mainC chan<- []extractedBook) {
	var books []extractedBook
	c := make(chan extractedBook)
	pageURL := "https://search.shopping.naver.com/book/search?bookTabType=ALL&pageIndex=" + strconv.Itoa(page) + "&pageSize=40&query=" + term + "&sort=REL"
